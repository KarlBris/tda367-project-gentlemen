...
public class PlayerModelTestOld {
	...
	public void setUp() throws Exception {
		...
		RuleController ruleController = Manager.instantiate(new RuleFactory());

		teamOne = Manager.instantiate(new TeamFactory());
		teamTwo = Manager.instantiate(new TeamFactory());
		...
		teamOneFlag = Manager.instantiate(new FlagFactory(), new Vector2f(0.0f,
				0.0f));
		teamTwoFlag = Manager.instantiate(new FlagFactory(), new Vector2f(0.0f,
				10.0f));

		pm = Manager.instantiate(new PlayerOneFactory()).getModel();
	...
	/*
	 * @Test public void
	 * testPickUpBallAndGetBallControllerAndIsCarryingBallAndThrowBall() {
	 * 
	 * // Test player have no ball assertTrue(pm.getBallController() == null);
	 ...
	 * 
	 * // Test should be success and return true assertTrue(pm.pickUpBall());
	 * 
	 ...
	 * 
	 * // Throw ball should return true assertTrue(pm.throwBall()); // Shouldent
	 * be carrying a ball anymore assertTrue(!pm.isCarryingBall()); // The ball
	 * should have a velocity assertTrue(bc.getModel().getBody().getVelocity().x
	 * != 0 || bc.getModel().getBody().getVelocity().y != 0);
	 * 
	 ...
	 * 0.0f)); // Test should generate false, because the ball is too far way to
	 * be // picked up assertTrue(!pm.pickUpBall());
	 * 
	 * // The player should not have a ball in memory
	 * assertTrue(pm.getBallController() == null);
	 * assertTrue(!pm.isCarryingBall()); }
	 * 
	 * @Test public void testPlayerKnockOut() { // Test at start the player
	 * ain't knocked out assertTrue(!pm.isKnockedOut());
	 * 
	 * pm.playerKnockOut(); // Timer should be reseted
	 * assertTrue(pm.getKnockedOutTimer() == 0.0f);
	 * 
	 * assertTrue(pm.isKnockedOut()); }
	 * 
	 * @Test public void testGetGeometry() { assertTrue(pm.getGeometry() !=
	 * null); }
	 * 
	 * @Test public void testGetBody() { assertTrue(pm.getBody() != null); }
	 * 
	 * @Test public void
	 * testPickUpFlagAndGetFlagControllerAndIsCarryingFlagAndDropFlag() { //
	 * Test player have no flag assertTrue(pm.getFlagController() == null);
	 * assertTrue(!pm.isCarryingFlag());
	 * 
	 * teamTwoFlag.setPosition(new Vector2f(Constants.FLAG_PICK_UP_DISTANCE,
	 * 0.0f)); pm.setPosition(new Vector2f(0.0f, 0.0f));
	 * 
	 * // Test should be success and return true assertTrue(pm.pickUpFlag());
	 * 
	 ...
	 * assertTrue(!teamTwoFlag.isPickUpAble()); // Drop should be success
	 * assertTrue(pm.dropFlag()); // The flag should be droped by the player
	 * assertTrue(teamTwoFlag.isPickUpAble()); // An other drop should fail
	 ...
	 * teamTwoFlag.setPosition(new Vector2f( (Constants.FLAG_PICK_UP_DISTANCE +
	 * 0.01f), 0.0f)); // Test should generate false, because the flag is too
	 * far way to be // picked up assertTrue(!pm.pickUpFlag());
	 * 
	 * // The player should not have a flag in memory
	 * assertTrue(pm.getFlagController() == null);
	 * assertTrue(!pm.isCarryingFlag()); }
	 * 
	 * @Test public void testCaptureEnemyFlag() { // Test player should not be
	 * carrying flag assertTrue(!pm.isCarryingFlag());
	 * 
	 * teamTwoFlag.setPosition(new Vector2f(20.0f, 10.0f)); pm.setPosition(new
	 * Vector2f(20.0f, 10.0f)); // The other teams flag should be picked up
	 * assertTrue(pm.pickUpFlag());
	 * 
	 * pm.setPosition(new Vector2f(Constants.FLAG_PICK_UP_DISTANCE + 0.01f,
	 * 0.0f)); teamOneFlag.setPosition(new Vector2f(
	 * Constants.FLAG_PICK_UP_DISTANCE + 0.01f, 0.0f)); // This should generate
	 * false, the flag ain't at its home position which // is (0.0f, 0.0f)
	 * assertTrue(!pm.captureEnemyFlag()); assertTrue(pm.isCarryingFlag());
	 * 
	 * teamOneFlag.returnFlagHome(); assertTrue(teamOneFlag.isAtHome());
	 * 
	 ...
	 * }
	 * 
	 * @Test public void testReturnTeamFlag() { teamOneFlag.returnFlagHome();
	 * pm.setPosition(new Vector2f(teamOneFlag.getHomePosition())); // The flag
	 * is already home assertTrue(!pm.returnTeamFlag());
	 * 
	 * teamOneFlag.setPosition(new Vector2f(Constants.FLAG_PICK_UP_DISTANCE,
	 * 0.0f)); // The flag is in range of the team player and the flag ain't at
	 * it's // home position assertTrue(pm.returnTeamFlag());
	 * assertTrue(teamOneFlag.isAtHome());
	 * 
	 * teamOneFlag.setPosition(new Vector2f( Constants.FLAG_PICK_UP_DISTANCE +
	 * 0.1f, 0.0f)); // The flag is out of range of the team player
	 * assertTrue(!pm.returnTeamFlag()); assertTrue(!teamOneFlag.isAtHome());
	 * 
	 * teamTwoFlag.setPosition(new Vector2f(0.0f, 0.0f));
	 * 
	 * // Pick up other teams flag assertTrue(pm.pickUpFlag());
	 * 
	 * teamOneFlag.setPosition(new Vector2f(Constants.FLAG_PICK_UP_DISTANCE,
	 * 0.0f)); // The player should be able to return the flag even if he is
	 * carrying a // flag of his own assertTrue(pm.returnTeamFlag());
	 * 
	 * }
	 */
	@Test
	...
	/*
	 * @Test public void testUpdateFlagPosition() { pm.setPosition(new
	 * Vector2f(0.0f, 10.0f)); assertTrue(pm.pickUpFlag()); Vector2f pos =
	 * teamTwoFlag.getPosition();
	 * 
	 * // Give player a new position pm.setPosition(new Vector2f(10.0f, 10.0f));
	 * pm.updateFlagPosition();
	 * 
	 * // The flag should have changed position
	 * assertTrue(!(Tools.distanceBetween(teamTwoFlag.getPosition(), pos) <=
	 * 0.01f)); }
	 * 
	 * @Test public void testSetTeamAndGetTeam() { assertTrue(pm.getTeam() ==
	 * teamOne); pm.setTeam(teamTwo); assertTrue(pm.getTeam() == teamTwo); }
	 */
