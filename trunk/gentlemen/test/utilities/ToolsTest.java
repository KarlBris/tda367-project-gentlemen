...
public class ToolsTest {
	...
	public void testVectorToAngle() {
		...
		// Test that a vector of {Float.NaN, 0.0f} should produce an angle of
		// 0.0
		testVector = new Vector2f(Float.NaN, 0.0f);
		testAngle = Tools.vectorToAngle(testVector);
		assertTrue(Tools.floatsEqual(Math.abs(testAngle), 0.0f));

		// Test that a vector of {Float.POSITIVE_INFINITY, 0.0f} should produce
		// an angle of 0.0
	...
	public void testRandomVectorInAreaVector2fVector2f() {
		// Test that the random vector that is created is not larger than
		// allowed
		...
		// Test that the random vector that is created is (0.0f, 0.0f), since
		// this is the only allowed vector
	...
	public void testRandomVectorInAreaVector2fVector2fVector2f() {
		...
		// Test that the random vector that is created is (0.0f, 0.0f), since
		// this is
		// the vector that is returned when the randomization has failed 100
		// times
		start = new Vector2f(0.0f, 0.0f);
		size = new Vector2f(0.0f, 0.0f);
